...
// Ver:		1.2.3 , Sat 13 Mar 16:05:36 2004
//
// License:	This code is released under JpGraph Professional License
//              http://www.aditus.nu/jpgraph/
//
// Copyright (c) 2003, 2004 Johan Persson
//
...
class BarcodeEncode_CODE128 extends BarcodeEncode {
    ...
    function EncodeSymbolC($aSym1,$aSym2) {
       $val = 10*$aSym1+$aSym2;    
       if( $val > 99 ) 
	   JpGraphError::RaiseL(1005,$aSym,$aSym2);
...
//("Internal encoding error. Trying to encode $aSym is not possible in Code 128");
    ...
    // Find out what character set is required
	...
	for( $i=0; ctype_digit(substr($aData,$i+$aPos,1)); ++$i )
	    ;
	if( $i >= 4 ) {
	...
	while( $i < $n ) {
		...
		case CODE128_B:
		    ...
		    break;
		case CODE128_C:
		    if( $i < $n-1 && ctype_digit(substr($aData,$i,2)) ) {
...
class OutputBackend_IMAGE extends OutputBackend {
	...
	// Hieght must be at least 15% of width
	if( $height < round(0.15*$totwidth) ) {
	...
	if( $aShowDetails ) {
	    ...
	    $g->img->SetTextAlign('left','top');
	    $g->img->SetFont(FF_FONT2);
   	    $g->img->StrokeText($textmargin,$topmargin,$s);
	}
