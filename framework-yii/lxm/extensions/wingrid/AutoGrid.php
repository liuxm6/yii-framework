...
class AutoGrid extends CInputWidget
{
    const ASSETS_NAME='/wingrid';
    public $json_url = '/json/autogrid';
    ...
    public function initWinGrid()
    {
        $this->validate();
        if (empty($this->url)) {
            $params = array('table'=>$this->className,'modelPath'=>$this->modelPath);
            if ($this->condition)
    ...
    public function validate()
    {
        ...
        //Yii::import($this->modelPath.".".$this->className);
        $table = CActiveRecord::model($this->className);
